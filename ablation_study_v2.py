...
N_PCA = 75  # FIXED to match the corrected classifier/SHAP setup
...
def build_features(config, stylo_train, stylo_test, emb_train, emb_test,
    ...
    pca = PCA(n_components=N_PCA, random_state=42).fit(emb_train_s)
    ...
    if config == 'stylo_only':
        ...
        return (np.hstack([stylo_train_s, emb_train_pca]), np.hstack([stylo_test_s, emb_test_pca]),
                np.hstack([stylo_light_s, emb_light_pca]), np.hstack([stylo_heavy_s, emb_heavy_pca]))

def make_classifier(use_ensemble):
    xgb = XGBClassifier(n_estimators=200, max_depth=6, learning_rate=0.05, random_state=42, eval_metric='mlogloss')
    if not use_ensemble:
        return xgb
...
print(f"\n=== Ablation Study (PCA={N_PCA}, corrected): 4 Configurations x 5 Folds ===\n")

for fold, (train_idx, test_idx) in enumerate(skf.split(X_stylo_orig, y_orig), 1):
    print(f"--- Fold {fold} ---")
    y_train, y_test = y_orig[train_idx], y_orig[test_idx]
    y_light_test = y_light_df_labels = df[df['edit_condition']=='light'].reset_index(drop=True)['model_label'].values[test_idx]
    y_heavy_test = df[df['edit_condition']=='heavy'].reset_index(drop=True)['model_label'].values[test_idx]

    for name, feat_type, use_ensemble in CONFIGS:
        X_train, X_test, X_light, X_heavy = build_features(
            feat_type, X_stylo_orig[train_idx], X_stylo_orig[test_idx],
...
print("\n\n=== ABLATION STUDY FINAL RESULTS (PCA=75, mean across 5 folds) ===\n")
print(f"{'Configuration':<45} {'Baseline':<12} {'Light':<12} {'Heavy':<12}")
print("-" * 81)
for name, _, _ in CONFIGS:
    b, l, h = np.mean(results[name]['baseline']), np.mean(results[name]['light']), np.mean(results[name]['heavy'])
    print(f"{name:<45} {b:<12.4f} {l:<12.4f} {h:<12.4f}")
print("\nChance baseline (4 classes): 0.2500")

pd.DataFrame([{'config': n, 'baseline': np.mean(results[n]['baseline']),
               'light': np.mean(results[n]['light']), 'heavy': np.mean(results[n]['heavy'])}
              for n, _, _ in CONFIGS]).to_csv('ablation_results_FINAL.csv', index=False)
print("Saved to ablation_results_FINAL.csv")
