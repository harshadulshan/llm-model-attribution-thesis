...
print("=== This is the SAME data, SAME random_state=42, SAME everything as train_classifier_v2.py ===")
print("=== The only change: EVERY fold's pipeline gets saved, not just fold 1 ===\n")
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X_stylo_orig, y_orig), 1):
    ...
    # Save THIS fold's complete fitted pipeline (all 5 folds now, not just fold 1)
    with open(f"fold{fold}_pipeline.pkl", "wb") as f:
        pickle.dump({
            'ensemble': ensemble, 'stylo_scaler': stylo_scaler, 'emb_scaler': emb_scaler,
            'pca': pca, 'train_idx': train_idx, 'test_idx': test_idx
        }, f)
    print(f"  -> Saved fold{fold}_pipeline.pkl")
...
print("\nThese numbers should match Table in Chapter 4 exactly (same data, same seed, same everything).")
print("If they don't match, STOP and tell Claude before deploying anything — something upstream changed.")
