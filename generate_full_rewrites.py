...
print(f"Light: Levenshtein-only pass = {df['pass_light'].mean()*100:.1f}% | Cosine-only pass = {df['cos_pass_light'].mean()*100:.1f}% | BOTH (final) = {df['final_pass_light'].mean()*100:.1f}%")
print(f"Heavy: Levenshtein-only pass = {df['pass_heavy'].mean()*100:.1f}% | Cosine-only pass = {df['cos_pass_heavy'].mean()*100:.1f}% | BOTH (final) = {df['final_pass_heavy'].mean()*100:.1f}%")
